...
bot = telebot.TeleBot('7259941456:AAFw1Kw1TLPhHvmdoUgriu_KDBOWWUzx0Ec')

# Admin user IDs
admin_id = ["7259941456","2062628281"]
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "Only @MAHADEVSANNIBot Can Run This Command."
    bot.reply_to(message, response)

@bot.message_handler(commands=['owner'])
def show_owner(message):
    response = "👑 Bot Owner: @MAHADEVSANNIBot"  # Replace with the actual owner username
    bot.reply_to(message, response)
        ...
        if len(command) > 2:  # Check if the command contains the user ID and time duration
            ...
            try:
                ...
                if user_to_add not in allowed_user_ids:
                    ...
                    save_user_access(user_access)
                    response = f"User {user_to_add} approved for {time_value} {time_unit} by @MAHADEVSANNIBot.\n\n\n 🅑🅞🅣 🅛🅘🅝🅚: @dedicating_bot"
                else:
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\n"
                    ...
                    "👉 Contact an Admin or the Owner @MAHADEVSANNIBot for approval.\n"
                    "🌟 Become a proud supporter and purchase approval.\n"
                    "💬 Chat with an Owner @MAHADEVSANNIBot now and level up your capabilities!\n\n"
...
def welcome_start(message):
    ...
    response = (
        f"🥀Welcome {user_name}!\n\n"
        " For User ID : /id \n\n"
        "👉 Join our official channel - @Mahadevsanni ✅\n\n"
        "👑 For access: @@Mahadevsanni"
        "👑 OWNER : @@Mahadevsanni"
    )
    bot.reply_to(message, response)
...
def show_access_expiry(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = (
            ...
            "👉 Contact an Admin or the Owner @Mahadevsanni for approval.\n"
            "🌟 Become a proud supporter and purchase approval.\n"
            "💬 Chat with an Owner @Mahadevsanni now and level up your capabilities!\n\n"
            "🚀 Ready to supercharge your experience? Take action and get ready for powerful attacks!"
        )
...
@bot.message_handler(commands=['owner'])
def show_owner(message):
    response = "👑 Bot Owner: @Mahadevsanni"  # Replace with the actual owner username
    bot.reply_to(message, response)
